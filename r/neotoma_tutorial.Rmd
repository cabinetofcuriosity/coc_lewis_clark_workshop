...
# If you are working on your own machine, you must first install these packages.
# install.packages(c("ggplot", "ggmap", "tidyverse", "skimr", "taxize", "neotoma", "purrr", "WikipediR", tidyr))
...
```{r,message=F}
## BONUS CHALLENGE: I want to be tidy and use purrr, but I can't figure out why the following doesn't work:
## toPlot = map(smilodon, ~.x$site.data) %>% map_df( `[`, c("long","lat"))
## Can you use `map` style function calls to get a dataframe toPlot that matches the one below?
...
3. Visualize the result of your `get_dataset` call. 

```{r}
## use this helper function to get a dataframe that makes plotting easier
## generalized from code above
helperFunction <- function(data){
  
  long <- unlist(lapply(data, function(x) {
  x$site.data$long
}))
lat <- unlist(lapply(data, function(x) {
  x$site.data$lat
}))
...
If you want to be fancy you can access Wikipedia from R. This won't work for every taxon name, but it's worth a try. 
...
It would be nice if we could do this automatically for every entry in `mammals`. I wasn't able to get this to work. If you are curious why, read on.

```{r,eval = F}
#https://github.com/Ironholds/WikipediR/issues/25

## I want to be able to do this
lapply(mammals$taxon.name, function(x) {
  page_content("en", "wikipedia", page_name = x, as_wikitext = T)
})


## failed attempt to get around the problem
wiki_wrapper <- function(x) {
  page_content("en", "wikipedia", page_name = x, as_wikitext = T)
}

mammals %>% map_df(safely(wiki_wrapper(~ .x$taxon.name), otherwise = NA_real_, quiet = T))
```
...
1. Add taxonomic classification to the dataset you mapped in the previous activity.
2. There is a lot of processing involved. What questions do you have about the process? Look at the documentation for functions that you aren't familiar with.
...
## [TODO] I needed one more number for the manual selection. I put NA and then get 26 not classified below.
...
[TODO] Switch from read/write to using this if you think this approach is ok.

```{r, eval=FALSE}
helper <- function(x,y){

  xNice = x %>% filter(rank %in% c("order", "family", "genus")) %>%
    select(-id) %>% complete(rank = c("order","family","genus"))
  
  y <- c(y, as.character(xNice$name))
  return(y)
}

## see why I'm removing 74 and 200 below
## [TODO] Check that these numbers stay the same after we deal with the discrepancy in manual selection key
extinct_tol_list = map2(extinct_tol[-c(74,200)], names(extinct_tol)[-c(74,200)],~helper(.x,.y)) 
tol_classification <- as.data.frame(do.call(rbind, extinct_tol_list))
names(tol_classification)= c("order", "family", "genus", "taxon.name")
...
which(unlist(lapply(mapply(helper,extinct_tol,names(extinct_tol),SIMPLIFY = F) , length))!=4)

extinct_tol[[74]] %>% filter(rank %in% c("order", "family", "genus")) %>%
    select(-id) %>% complete(rank = c("order","family","genus"))
## double family

extinct_tol[[200]] %>% filter(rank %in% c("order", "family", "genus")) %>%
    select(-id) %>% complete(rank = c("order","family","genus"))
## double order
```
