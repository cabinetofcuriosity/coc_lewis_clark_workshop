...
## Can you use `map` style function calls to get a dataframe to Plot that matches the one below?
...
Now we want to extract  `order`, `family`, and `genus` in `clean_mammoth_test` and merge `mammoth_test`. Do not be intimidated if this code looks hard.  Take it one line at a time. 
...
**Note**: You can really do this with any subset of the Neotoma mammals dataset, but if you do it with the whole thing, it takes a long time and you need a tad more cleanup. Take each step one at a time. 
...
3,0012 (number may change) bone samples in our data set come from extinct animals. Now that we have a nice subset of our species, let's go ahead and use `taxize` to merge in genus, order, and family details for each sample.
